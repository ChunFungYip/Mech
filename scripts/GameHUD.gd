...
func _build_ui() -> void:
	...
	add_child(viewport_root)

	var root := Control.new()
	root.name = "ResponsiveDesignCanvas"
	root.size = DESIGN_SIZE
	root.mouse_filter = Control.MOUSE_FILTER_IGNORE
	viewport_root.add_child(root)
	_design_root = root
...
func _update_responsive_layout() -> void:
	...
	_design_root.scale = Vector2.ONE * fit_scale
	_design_root.position = (viewport_size - DESIGN_SIZE * fit_scale) * 0.5
...
func _process(delta: float) -> void:
	...
	var lock_position: Vector2 = player.call("get_locked_enemy_screen_position")
	var show_lock_reticle: bool = missile_selected and lock_position.x >= 0.0 and lock_position.y >= 0.0
	_lock_reticle.visible = show_lock_reticle
	if show_lock_reticle:
		var lock_center := Vector2(lock_position.x * DESIGN_SIZE.x, lock_position.y * DESIGN_SIZE.y)
		var reticle_half := LOCK_RETICLE_SIZE * 0.5
	...
	var show_part_health: bool = missile_selected and not locked_boss and locked_enemy != null and is_instance_valid(locked_enemy) and locked_enemy.has_method("get_part_health_snapshot")
...
func _build_lock_reticle(root: Control) -> void:
	...
	_lock_reticle.name = "MissileLockReticle"
	_lock_reticle.size = LOCK_RETICLE_SIZE + Vector2(0.0, 24.0)
	_lock_reticle.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_lock_reticle.visible = false
	root.add_child(_lock_reticle)

	var reticle_color := Color(1.0, 0.24, 0.10, 0.96)
	var thickness := 3.0
	var arm_length := 24.0
	_add_reticle_segment(Vector2(0.0, 0.0), Vector2(arm_length, thickness), reticle_color)
	_add_reticle_segment(Vector2(0.0, 0.0), Vector2(thickness, arm_length), reticle_color)
	_add_reticle_segment(Vector2(LOCK_RETICLE_SIZE.x - arm_length, 0.0), Vector2(arm_length, thickness), reticle_color)
	_add_reticle_segment(Vector2(LOCK_RETICLE_SIZE.x - thickness, 0.0), Vector2(thickness, arm_length), reticle_color)
	_add_reticle_segment(Vector2(0.0, LOCK_RETICLE_SIZE.y - thickness), Vector2(arm_length, thickness), reticle_color)
	_add_reticle_segment(Vector2(0.0, LOCK_RETICLE_SIZE.y - arm_length), Vector2(thickness, arm_length), reticle_color)
	_add_reticle_segment(Vector2(LOCK_RETICLE_SIZE.x - arm_length, LOCK_RETICLE_SIZE.y - thickness), Vector2(arm_length, thickness), reticle_color)
	_add_reticle_segment(Vector2(LOCK_RETICLE_SIZE.x - thickness, LOCK_RETICLE_SIZE.y - arm_length), Vector2(thickness, arm_length), reticle_color)
	var lock_label := _label(_lock_reticle, "LOCKED", Vector2(0.0, LOCK_RETICLE_SIZE.y + 2.0), Vector2(LOCK_RETICLE_SIZE.x, 20.0), 10, reticle_color)
	lock_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER

func _add_reticle_segment(position: Vector2, size: Vector2, color: Color) -> void:
	var segment := ColorRect.new()
	segment.position = position
	segment.size = size
	segment.color = color
	segment.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_lock_reticle.add_child(segment)
